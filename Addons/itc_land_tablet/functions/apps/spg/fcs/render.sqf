...
[_display, 1017, str (round _vehicleDirMils)] call itc_land_tablet_fnc_set_text;
[_display, 1018, str round (_turretDirMils + 3200)] call itc_land_tablet_fnc_set_text;
...
[_display, 1019, str round _realElevation] call itc_land_tablet_fnc_set_text;

_ammo = (configFile >> "CfgMagazines" >> _magazine >> "displayNameShort")  call BIS_fnc_getCfgData;

[_display, 1021, _ammo] call itc_land_tablet_fnc_set_text;

[_display, 1025, str _ammoCount] call itc_land_tablet_fnc_set_text;
...
if(count _solutions > 0) then {
  ...
  _solutionString = _solutionString + format ["DEFLECTION %1<br/>",round _df];
  ...
  _solutionString = _solutionString + format ["IMPACT ANGLE %1<br/>",round _impAng];
