...
_randomizedVelocity = _velocity vectorAdd [random 50, random 50, random 50];
...
[{
  ...
  if(time > _firedTime + 4 && typeOf _projectile == "Land_FoodContainer_01_F") then { //OPEN THE PARACHUTE
    //player sideChat "PARACHUTE DEPLOYED";
  ...
  if(time > _firedTime + 8 && typeOf _projectile == "ITC_155Extra") then { //SEARCH FOR A TARGET
    ...
    _selectTankIndex = _tanks findIf {(alive _x && _x distance _projectile < 50)};
    if(_selectTankIndex != -1) then {
      _selectTank = _tanks # _selectTankIndex;
      //player sideChat "TARGET FOUND";
      ...
      //player sideChat str _vectorToTank;
      deleteVehicle _projectile;
      [_pfhId] call CBA_fnc_removePerFrameHandler;
    };
  };

  if(getPosATL _projectile # 2 < 20 || getPosASL _projectile # 2 < 20) exitWith {
    //player sideChat "SELF DESTRUCT";
