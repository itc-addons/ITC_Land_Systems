		   ...
		   private _displayedDir = [_weaponDir, "mil4", true] call ace_mk6mortar_fnc_dev_formatNumber; //Take weapon direction in degrees, convert to mils and format as 4 figure string
			...
			if (itc_land_IGS_showAutoloader) then {
				...
				if (!(isNil "itc_land_sphloadersettings") && {count itc_land_sphloadersettings > 0}) then {
					(_display displayCtrl 1702) ctrlSetText (format ["LOAD: %1",((itc_land_sphloadersettings # 0) # 0)]);
					(_display displayCtrl 1703) ctrlSetText (format ["FUZE: %1", toUpper ((itc_land_sphloadersettings # 1) # 0)]);
					(_display displayCtrl 1704) ctrlSetText (format ["GUIDANCE: %1", toUpper ((itc_land_sphloadersettings # 2) # 0)]);				
				} else {
